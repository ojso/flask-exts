...
def test_form_rules(app, client, db, admin):
...
def test_rule_macro(app, client, db, admin):
    with app.app_context():
        Model1, _ = create_models(db)
        db.create_all()
...
def test_rule_container(app, client, db, admin):
    with app.app_context():
        Model1, _ = create_models(db)
        db.create_all()
...
def test_rule_header(app, client, db, admin):
    with app.app_context():
        Model1, _ = create_models(db)
        db.create_all()
...
def test_rule_field_set(app, client, db, admin):
    with app.app_context():
        Model1, _ = create_models(db)
        db.create_all()
...
def test_rule_inlinefieldlist(app, client, db, admin):
    with app.app_context():
...
def test_inline_model_rules(app, client, db, admin):
    with app.app_context():
        Model1, Model2 = create_models(db)
        db.create_all()
