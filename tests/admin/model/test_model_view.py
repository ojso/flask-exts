...
class MockModelView(ModelView):
    def __init__(
        ...
        **kwargs,
        ...
        if data is None:
            self.all_models = {1: MockModel(1), 2: MockModel(2)}
        else:
...
def test_view():
    ...
    assert view.model == MockModel
    assert view.name == "Mock Model"
    assert view.endpoint == "mockmodel"
    ...
    assert ("/details/", "details_view", ("GET",)) in view._urls
    assert ("/edit/", "edit_view", ("GET", "POST")) in view._urls
    assert ("/delete/", "delete_view", ("POST",)) in view._urls
    assert ("/export/<export_type>/", "export", ("GET",)) in view._urls
    assert ("/ajax/lookup/", "ajax_lookup", ("GET",)) in view._urls
    assert ("/ajax/update/", "ajax_update", ("POST",)) in view._urls
...
def test_modal_edit_bs4(client, admin):
    edit_modal_on = MockModelView(
        MockModel, name="edit_modal_on", edit_modal=True, endpoint="edit_modal_on"
    )
    edit_modal_off = MockModelView(
        MockModel, name="edit_modal_off", edit_modal=False, endpoint="edit_modal_off"
    )
    create_modal_on = MockModelView(
        MockModel, name="create_modal_on", create_modal=True, endpoint="create_modal_on"
    )
    create_modal_off = MockModelView(
        MockModel,
        name="create_modal_off",
        create_modal=False,
...
def test_export_csv(client, admin):
    ...
    view_data = {
        1: MockModel(1, "col1_1", "col2_1"),
        2: MockModel(2, "col1_2", "col2_2"),
        3: MockModel(3, "col1_3", "col2_3"),
    }
    ...
    view_data_v3 = {
        1: MockModel(1, "col1_1", 1),
        2: MockModel(2, "col1_2", 2),
        3: MockModel(3, None, 3),
    }
