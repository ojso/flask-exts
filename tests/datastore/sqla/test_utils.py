...
from ...models.model_normal import ModelNormal
...
def test_multiple_pk(app):
    with app.app_context():
        ...
        m4 = ModelMultpk(id=2, id2=2, data="fourth")
        db.session.add(m1)
        db.session.add(m2)
        db.session.add(m3)
        db.session.add(m4)
        ...
        # select pk values
        stmt = stmt_select_model_pk_values(ModelMultpk)
        # print(stmt)
        rows = db.session.execute(stmt).all()
        ...
        # delete by pk ids
        ids = [(1, 1), (1, 2)]
        stmt = stmt_delete_model_pk_ids(ModelMultpk, ids)
        # print(stmt)
